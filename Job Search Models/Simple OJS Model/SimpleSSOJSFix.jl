...
function SimulateProductivity(T :: Int64, PIZ :: Array, Zt :: Array)
    ...
    srand(123)              # Reset the random number generator to use the seed given by "seed"
    dU      = Uniform(0,1)
    ...
    drw     = convert(Array{Int64,1},squeeze(zeros(nsim,1),2)) # The j's will be the TFP realizations.
    drw[1]  = s0
    for k in 2 : nsim
...
function SolveEqmTheta(x0 :: Float64, nstate :: Int64, z0_pdf :: Array, psi :: Array, u :: Float64,
                       Soln_Values :: SolutionValues,  Params :: ParameterValues)
    rho = Params.rho
    chi = Params.chi
    b   = Params.b
    kappa = Params.kappa
    ...
    while crt > tol
        ...
        w  = chi*Zt + (1-chi)*b + chi*kappa + (1-chi)*rho*(mu-eta)*(1-delta)*sum(z0_pdf.*W) - (1-chi)*mu*rho*(1-delta)*SW - 
                (1-chi)*rho*(mu*(1-delta) - delta - mu*(1-Fw))*U + (1-chi)*eta*(1-delta)*rho*U
        TU = b + rho*mu*(1-delta)*sum(z0_pdf.*W) + rho*(1-mu*(1-delta))*U
        TW = w + rho*mu*(1-delta)*SW + rho*(1-delta)*(1-mu*(1-Fw)).*PIZ*W + rho*delta*(1-mu*(1-Fw))*U
    ...
    Soln_Values = SolutionValues(U,W,J,w,theta,psi,u)
    cdf_psi = cumsum(psi)
    res = kappa - rho*(1-delta)*eta*u*sum(z0_pdf.*J) - rho*(1-delta)*eta*(1-u)*sum(z0_pdf.*cdf_psi.*J)
    return res
end

function SimulateEconomy(nstate :: Int64, theta :: Float64, u0 :: Float64, psi0 :: Array,
                         z0_pdf :: Array{Float64,2}, PIZ :: Array{Float64,2}, Params :: ParameterValues)
    delta = Params.delta
    ...
    u    = u0
    psi_t = psi0*(1-u)  # psi_t is the histogram of employed individuals.  psi is the distribution of workers
                        # so that it sums to one whereas psi_t sums to the measure of employed.
    tol  = 1e-8
    crt  = 1
    bean = 0
    while crt > tol
        ...
        OJS_Inflows = sum(OJS_TRNS*(1-u),2)
    ...
    psi = psi_t/(1-u)
    println("The sum of unemployed and psi_t is: $(sum(psi_t)+u)")
    return psi_t, psi, Job2JobRate, u
end

function SolveEqm(theta0 :: Float64, u0 :: Float64, psi0 :: Array, z0_pdf :: Array, Soln_Values :: SolutionValues,  Params :: ParameterValues)
    theta = theta0
    psi = psi0
    u   = u0
    Job2JobRate = 0
    ...
    while crt > tol
        x0  = log(theta)
        @time xx = fcsolve(SolveEqmTheta,x0,[],nstate,z0_pdf,psi,u,Soln_Values,Params)
        theta_tmp  = exp(xx)
        @time psi_t, psi_tmp, Job2JobRate, u_tmp = SimulateEconomy(nstate, theta, u0, psi0, z0_pdf, PIZ, Params)
        crt = maximum(abs.([u ; psi_t'] - [u_tmp ; psi_t'])./maximum(1+abs.([u_tmp ; psi_t'])))
        println(crt)
        theta = theta_tmp
        psi   = psi_tmp
        u     = u_tmp
    end
    return theta, u, psi, Job2JobRate
end
...
kappa = 1.2     # Vacancy creation cost
...
load_sol = 1
if load_sol == 1
    Soln_Values = load("/Users/jacobwong/Dropbox/Jake's Julia Codes/Post-Version 5.0/Job Search Models/Simple OJS Model/SimpleSSOJSModelSoln.jld","Soln_Values")
    U   = Soln_Values.U
    W   = Soln_Values.W
    J   = Soln_Values.J
    w   = Soln_Values.w
...
for i in 1 : nstate
    if i < nstate
        Fw0[i] = sum(z0_pdf[i+1:nstate])  # Probability of worker leaving by OJS conditional on having an offer.
    end
end
if load_sol == 0
    theta0 = 0.6
    mu0  = JobFindingProb(theta0,Params)
    psi0 = 1/nstate*ones(1,nstate)
    # tld_psi = psi0
    # tld_delta = (1-mu0*tld_psi*Fw0)*delta
    # u0_tmp = tld_delta/(mu0 + tld_delta)
    u0    = delta/(mu0 + delta)
    # u0   = u0_tmp[1]
    Soln_Values = SolutionValues(U,W,J,w,theta0,psi0,u0[1])
else
    theta0 = Soln_Values.theta
    psi0   = Soln_Values.psi
    u0     = Soln_Values.u
end
theta, u, psi, Job2JobRate = SolveEqm(theta0, u0, psi0, z0_pdf, Soln_Values, Params)
mu = JobFindingProb(theta, Params)
eta = mu/theta
U, W, J, w, theta = ReconstructSolution(log(theta), nstate, z0_pdf, Params)
Soln_Values = SolutionValues(U,W,J,w,theta0,psi,u)
tld_psi = psi/(1-u)

save_sol = 1
if save_sol == 1
    save("/Users/jacobwong/Dropbox/Jake's Julia Codes/Post-Version 5.0/Job Search Models/Simple OJS Model/SimpleSSOJSModelSoln.jld","Soln_Values",Soln_Values,"Params",Params)
end
mean_wage = sum(psi*w)
...
if plot_figs == 1
    ...
    title("The Value of Unemployment",fontdict=font1)
    axis([Zt[1],Zt[nstate],U-0.1*U,U+0.1*U])
    subplot(2,2,2)
    plot(Zt,W,linewidth=2,color="b")
    #xlabel(L"$z$",fontdict=font1)
    ylabel(L"$W$",fontdict=font1)
    title("The Value of Employment",fontdict=font1)
    axis([Zt[1],Zt[nstate],minimum(W),maximum(W)])    
    subplot(2,2,3)
    plot(Zt,J,linewidth=2,color="b")
    xlabel(L"$z$",fontdict=font1)
    ylabel(L"$J$",fontdict=font1)
    title("The Value of the Firm",fontdict=font1)
    axis([Zt[1],Zt[nstate],minimum(J),maximum(J)])
    subplot(2,2,4)
    plot(Zt,w,linewidth=2,color="b")
    xlabel(L"$z$",fontdict=font1)
    ylabel(L"$w$",fontdict=font1)
    title("Wages",fontdict=font1)
    axis([Zt[1],Zt[nstate],minimum(w),maximum(w)])

    fig = figure("Distribution_of_Workers")
    plot(w,psi',linewidth=2,color="b",label="Observed Wage Distribution")
    plot(w,z0_pdf,linewidth=2,color="r",label="Distribution of Wage Offers")
    legend()
    xlabel("w")
    axis([w[1],w[nstate],0,max(maximum(z0_pdf),maximum(psi'))])
