function DMP_DRW_JD_Simulate(nsim :: Int64, Brn :: Int64, THETA :: Array{Float64},
    ...
    #-----------------------------------------
    sA0     = ceil(nstateD/2)  # Set the initial state to equal the state with the median of the labour 
                               # productivity process.  (There are nstateD states for labour productivity)
    ...
    p       = rand(dU,T)    # Draw T realizations of a random variable that is uniformly distributed over the [0,1] interval.  These
                            # can be treDted as probabilities.
