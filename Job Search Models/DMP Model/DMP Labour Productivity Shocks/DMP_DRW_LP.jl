...
include("DMP_DRW_LP_SSS.jl")
include("DMP_DRW_LP_SRes.jl")
include("DMP_DRW_LP_TRes.jl")
include("DMP_DRW_LP_Simulate.jl")
...
mA    = 0
b     = 0.4*exp(mA) # Unemployment consumption
...
Param = Parameter_Values(rho,beta,delta,alpha,z,b,mA,nstateA,eAt,PIA)
...
x0_ss   = 1
thss    = fcsolve(DMP_DRW_SS,x0_ss,[],Param)
...
THETA = dropdims(zeros(nstateA,1),dims=2)

solve = 1
if solve == 1
    soln_method = 0
    if soln_method == 1
        # In the function "DMP_DRW_TRes", theta values will be constructed as 
        ...
        # thss*ones(nstateA,1) = exp.(x0).  Take the log of both sizes and 
        # x0 = log(thss*ones(nstateA,1))
        x0        = log.(thss*ones(nstateA,1))  # This creates an Array{Float64,2} 
                                                # and fcsolve requires x0 to be a 
                                                # vector so...
        # Create an Array{Float64,1} type by dropping the second dimension of x0.
        x0        = dropdims(x0,dims=2) 
        options   = [100 ; 1e-8 ; 1e-7 ; 1e-3 ; 1]
        Txx       = fcsolve(DMP_DRW_TRes,x0,[],Param)  # If solving using the equation without the Surplus as a variable
        global THETA     = exp.(Txx)  # This ensures that theta is always strictly positive.
    else
        # In the function "DMP_DRW_SRes", S values will be constructed as SMat = exp.(x0).  A good initial guess for 
        # the values of total match surplus outside (but near) the non-stochastic steady state is Sss.  So that a good initial
        # value for the elements of x0 are Sss*ones(nstateA,1) = exp.(x0).  Take the log of both sizes and 
        # x0 = log(Sss*ones(nstateA,1))
        x0        = log.(Sss*ones(nstateA,1))  # This creates an Array{Float64,2} and fcsolve requires x0 to be a vector so...
        x0        = dropdims(x0,dims=2)         # Create an Array{Float64,1} type by dropping the second dimension of x0.
        Sxx       = fcsolve(DMP_DRW_SRes,x0,[],Param)  # If solving using the Surplus as a variable
        # Reconstruct equilibrium theta's using the solution to S(a)
        SMat    = exp.(Sxx)  # Make sure that the surplus of a job is strictly positive
        ES      = zeros(nstateA,1)
        for i in 1 : nstateA
            tmp = 0
            for ii in 1 : nstateA
        ...
        global THETA   = (((rho*(1-beta)*ES)/z).^(alpha).-1).^(1/alpha)  # This creates an Array{Float64,2} type and I want to
                                                                         # vectorize it so...
        global THETA   = dropdims(THETA,dims=2)
    end
end
...
if plotopt == 1
    ...
    figure
    subplot(2,3,1)
    plot(collect(1:T),(log.(lpvec) .- log(exp(mA)))*100,linewidth=1,color="b")
    title("Productivity",fontdict=font2)
    ...
    subplot(2,3,3)
    plot(collect(1:T),log.(fndvec./jfndsss),color="b")  # 0.1442 is the HM job finding rate in the stochastic steady state
    title("Job Finding Probability",fontdict=font2)
    xlabel("Quarters",fontdict=font1);
    #ylabel("# Deviation from Stochastic Steady State",fontdict=font1)
    #set(gca,"xlim",[1,T],"fontname","times","fontsize",12);
    subplot(2,3,4);
    plot(collect(1:T),log.(fllvec./jfllsss),color="b")  # 0.2254 is the HM job filling rate in the stochastic steady state
    ...
    uvec, vvec, thvec, hvec, fndvec, fllvec, wvec, ChainA = DMP_DRW_LP_Simulate(nsim, Brn, THETA, SSSvec, Param)

    T = nsim

    figure
    subplot(2,3,1)
    plot(collect(1:T),(log.(ChainA) .- log(exp(mA)))*100,linewidth=1,color="b")
    title("Productivity",fontdict=font2)
    xlabel("Quarters",fontdict=font1)
    ylabel("% Deviation from Stochastic Steady State",fontdict=font1)
    subplot(2,3,2)
