...
THETA   = (((rho*(1-beta)*ES)/z).^(alpha).-1).^(1/alpha)  # This creates an Array{Float64,2} type and I want to
                                                          # vectorize it so...
THETA   = dropdims(THETA,dims=2)
...
if plotopt == 1
    ...
    subplot(2,3,2)
    plot(collect(1:T),log.(uvec./usss),linewidth=1,color="b")
    title("Unemployment Rate",fontdict=font2)
    xlabel("Quarters",fontdict=font1)
    subplot(2,3,3)
    plot(collect(1:T),log.(fndvec./jfndsss),linewidth=1,color="b")
    title("Job Finding Probability",fontdict=font2)
    xlabel("Quarters",fontdict=font1)
    subplot(2,3,4);
    plot(collect(1:T),log.(fllvec./jfllsss),linewidth=1,color="b")
