function DMP_DRW_Simulate(nsim :: Int64, Brn :: Int64, THETA :: Array{Float64}, SSSvec  :: Array{Float64,1}, Param :: Parameter_Values)
    ...
    #--------------------------------------------
    # Simulate a draw from the two Markov Chains:
    #--------------------------------------------
    ...
    # Repeat above and simulate a sequence for the job separation rate.
    sD0     = ceil(nstateD/2); # Set the initial state to equal the state with 
                               # the median of the separation rate process. (There are nstateD states.)

    Random.seed!(50417)     # Store the seed for the vector of random numbers to be used in the simulations
    dU      = Uniform(0,1)
    p       = rand(dU,T)    # Draw T realizations of a random variable that is uniformly distributed over the [0,1] interval.  These
                            # can be treated as probabilities.
    drw     = convert(Array{Int64,1},dropdims(zeros(T,1),dims=2))    # The j's will be the TFP realizations.
    drw[1]  = sD0
    for k in 2 : T
        drw[k]    = minimum(findall(cumPID[drw[k-1],:] .> p[k]))
    end
    ChainD  = eDt[drw]
    # Now construct a vector indicating the state for TFP in each period
    statevecD   = zeros(T,1);
    ...
    statevecD = convert(Array{Int64,1},dropdims(statevecD,dims=2))
    ...
    return uvec[Brn+1:T], vvec[Brn+1:T], thvec[Brn+1:T], hvec[Brn+1:T], fndvec[Brn+1:T], fllvec[Brn+1:T], 
           wvec[Brn+1:T], ChainA[Brn+1:T], ChainD[Brn+1:T]
end
