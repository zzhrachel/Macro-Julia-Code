# This is an example of how to use fcsolve.jl.
using Distributions
using Printf
using LinearAlgebra
...
function fcsolve(FUN :: Function, x :: Real, options :: Array, VarArgIn...)
    ...
    # The algorithm above will return x as a Vector.  As the input in this dispatch is a Real number, return a real number by 
    # extracting the first element of the 1x1 Vector x:    
    return x
end

# Use if solving multivariate case:
function fcsolve(FUN :: Function, x :: Array{Float64,1}, options :: Array, VarArgIn...)
	# Solves for FUN(x,P1,P2,...)=0
	#
	# FUN should be written - 	so that parametric arguments are packed in to vectors P1,...
	#	 					- 	so that if presented with a matrix x, it produces a return value of
